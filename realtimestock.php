 ...
 <div class="container">
  <div class="row">
      ...
      <th>ActionTime</th>
		...
		$sql ="SELECT ID,Name_of_item,Number_of_items,Purchasing_price_unit,Sale_price_unit,Difference_in_unit_price,actiontime from stock";
		...
		if($result-> num_rows > 0){
			while ($row=$result->fetch_assoc()) {
             $opt=generateOptions($row['Name_of_item'],$optionsArray);
				...
				echo "<tr><td>".$row["Name_of_item"]."</td><td>".$row["Number_of_items"]."</td><td>".$row["Purchasing_price_unit"]."</td><td>".$row["Sale_price_unit"]."</td><td>".$row["Difference_in_unit_price"]."</td><td>".$row["actiontime"]."</td></tr>";
			}
			echo "</table>";
		}
		else{
